...
% --- DIV labels defined here (after load, so no .mat can overwrite them) ---
div_labels_new = {'DIV 59', 'DIV 54', 'DIV 52', 'DIV 47'};
div_labels_old = {'DIV 60', 'DIV 55', 'DIV 53', 'DIV 48'};
lbl_new = div_labels_new{p};
lbl_old = div_labels_old{p};
...
xlabel('Time (s)'); ylabel('Frequency (Hz)');
title(['Old Spec – ' lbl_old]); colorbar;

title(tl, ...
    ['LFP signal and spectrogram: New (' lbl_new ') vs Old (' lbl_old ')'], ...
    'FontSize', 13);

fid = fopen(fullfile(fig2C3_folder, 'representative_electrode_info.txt'), 'w');
fprintf(fid, 'NEW  → well: %s, electrode: %s (%s)\n', repWell_new, repElec_new, lbl_new);
fprintf(fid, 'OLD  → well: %s, electrode: %s (%s, %s)\n', repWell_old, repElec_old, lbl_old, repSys_old);
fclose(fid);

exportgraphics(fig, fullfile(fig2C3_folder, 'LFP_Spectrogram_NewVsOld_2x4_DIV60_v2.png'), 'Resolution', 300);
savefig(fig,        fullfile(fig2C3_folder, 'LFP_Spectrogram_NewVsOld_2x4_DIV60_v2.fig'));
close(fig);
