...
% Index matches panels(): 1=DIV60, 2=DIV55, 3=DIV53, 4=DIV48
...
ylabel({'New system'; lbl_new; 'Amplitude (µV)'}, 'FontWeight', 'bold');
...
ylabel({'Old system'; lbl_old; 'Amplitude (µV)'}, 'FontWeight', 'bold');
xlim([t_start t_end]); ylim(raw_ylim); grid on;
xlabel('Time (s)');
...
for pp = 1:4
    ...
    % Title shows both DIVs: New / Old
    title([div_labels_new{p} ' (New) / ' div_labels_old{p} ' (Old)']);
    xlabel('Frequency (Hz)');
